...
export class SettingController {
    ...
    @ApiNotFoundResponse({description:'Not found',status: HttpStatus.NOT_FOUND})
    @ApiQuery({name:'name',  required: false})
    @ApiQuery({name:'property', required: false})
    @ApiQuery({name:'type', enum:['string','number','array', 'boolean','map'], required: false})
    @ApiQuery({name:'object', enum:['users','roles','profiles', 'company','clients','cars','holdings','mail'],required: false})
    async get(
       ...
       @Query('property') property: string
        ...
        let settingDto = {name: name, type: type, object: obj, property: property}
